#!/usr/bin/env python
"""
Standalone synthetic search utility for FAISS indexes.
This script works independently without requiring the full memories.synthetic package.
"""
...
class EnhancedFAISSSearch:
    """
    Enhanced search layer for FAISS indexes with prioritization and metadata enrichment.
    """
    ...
    async def prioritized_search(self, 
                    query: str, 
        ...
        This search prioritizes tiers and filters by similarity threshold.
    ...
    def _infer_query_capabilities(self, columns: List[str], data_type: str) -> Dict[str, Any]:
        ...
        This enriches the raw FAISS results with inferred query capabilities
...
async def main():
    ...
    parser = argparse.ArgumentParser(description='Enhanced Search for FAISS indexes')
    ...
    search = EnhancedFAISSSearch(similarity_threshold=args.threshold)
    ...
    # Perform the search
    print(f"Running prioritized search for: '{args.query}' with threshold {args.threshold}")
    print(f"Searching tiers (in order): {', '.join(tiers_list)}")
    
    results = await search.prioritized_search(
        query=args.query,
        tiers=tiers_list,
        k=args.max_results,
...
# Helper function to run the prioritized search directly
async def run_search(query, threshold=0.7, k=5, stop_on_first_match=True):
    """
    Run a prioritized search for the given query.
    
    Args:
        query: Search query string
        threshold: Similarity threshold (0.0-1.0)
        k: Number of results per tier
        stop_on_first_match: Whether to stop at the first tier with matches
        
    Returns:
        Enhanced metadata results
    """
    search = EnhancedFAISSSearch(similarity_threshold=threshold)
    await search.initialize_all_tiers()
    ...
    if results:
        return await search.get_enhanced_metadata(results)
    return {}
