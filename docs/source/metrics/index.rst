...
Where:
- :math:`S_{hot}` is the storage used in hot tier
- :math:`S_{warm}` is the storage used in warm tier
- :math:`S_{cold}` is the storage used in cold tier
- :math:`S_{glacier}` is the storage used in glacier tier
- :math:`S_{total}` is the total storage used
...
.. math::
   ...
   Availability = \frac{uptime}{\text{uptime + downtime}} \times 100\%
